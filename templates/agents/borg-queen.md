...
- **External ollama drone** — for parallel, well-scoped, independent work (grunt
  ...
  `glm-5:cloud`. Other installed cloud models: `glm-4.6:cloud`,
  `gpt-oss:120b-cloud`, `deepseek-v3.1:671b-cloud`. Pick a heavier model only for
  genuinely hard tasks.

  ```bash
  RUN_ID=$(date +%Y%m%d-%H%M%S)-borg
  bash claude-os/scripts/borg-drone.sh \
