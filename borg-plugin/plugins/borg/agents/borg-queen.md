...
description: |
  ...
  ollama-backed Claude processes), and unifies them through a shared memory backend.
...
processes alike — read from and write to the **same knowledge bases**. That shared
store IS the hive mind.
...
- Reach it via the `mcp__code-forge__*` tools (you have them, when the Claude OS MCP
  is connected) and over HTTP at `CLAUDE_OS_API` (default `http://localhost:8051`).
  Drones without MCP use `curl`; helpers live in
  `${CLAUDE_PLUGIN_ROOT}/scripts/borg-lib.sh`.

The memory backend defaults to **Claude OS** but is pluggable: any backend
implementing the HTTP contract documented in this plugin's `README.md` works.

**The Borg protocol — every drone, every task, no exceptions:**
1. **RECALL** — before acting, query the collective for prior knowledge
   (`mcp__code-forge__search_all_knowledge_bases`, or `borg_recall`, query = the
   task). Treat hits as established truth from sibling drones.
...
1. **Confirm the collective is online:** `curl -sf "${CLAUDE_OS_API:-http://localhost:8051}/health"`.
   If down, tell the user to start their memory backend first. Ensure
   `borg-collective` exists (`mcp__code-forge__list_knowledge_bases`; create if missing).
